    ...
    """
        ...
        for todfact in todo_reporte:
            ...
            if   todfact.inventario.name != False :  
             worksheet.write(tf + ini, 15, todfact.inventario.name)
...
class Menu(models.Model):
     ...
     def _visible_menu_ids(self, debug=False):
         ...
         nombre_menu=""
         if self.env.user.ver_costo == False :  
             nombre_menu='Kardex Inventory Products - Admin'
         if self.env.user.ver_costo == True :   
             nombre_menu='Kardex Inventory Products - Asistant' 

         query_menu = """
         select * from ir_ui_menu where name  = '"""+ nombre_menu +"""'
         
         """
         query_menu_param = (nombre_menu,)
         self.env.cr.execute(query_menu, query_menu_param)
         movim = self.env.cr.dictfetchall()
         id_menu=0 
         for mov in movim:
           id_menu=  mov['id']
