...
class kardex_productos_inventario2(models.TransientModel):
    ...
    def _action_imprimir_grupo_inventario_excel(self, ):
        ...
        #Vamos agrupando por grupo de productos     
            query_grupos = """
        ---------agrupar productos
select pp.default_code,pp.id,pt.name,pt.categ_id  from product_product  pp
...
where pt.categ_id = %s
    """
            ...
            for prod in g:
                ...
                worksheet.write(4+ conteo, 8, _('Origin'), column_heading_style)
                worksheet.write(4+ conteo, 9, _('Pickin'), column_heading_style)
                worksheet.write(4+ conteo, 10, _('Invoice'), column_heading_style)
                worksheet.write(4+ conteo, 11, _('Inventory'), column_heading_style)
                ...
                for mov in movim: 
                    ...
                    worksheet.write(tm + ini2+ conteo, 1, mov['dat_cre'])
                    ##################################
                    ...
                    inventory_desc = self.env['stock.inventory'].search([('id', '=', mov['inventory_id'])])  
                    worksheet.write(tm + ini2+ conteo, 15, inventory_desc.name)
    ...
    """
        ...
        worksheet.write(4, 8, _('Origin'), column_heading_style)
        worksheet.write(4, 9, _('Pickin'), column_heading_style)
        worksheet.write(4, 10, _('Invoice'), column_heading_style)
        worksheet.write(4, 11, _('Inventory'), column_heading_style)
        ...
        for todfact in todo_reporte:
            ...
            if   todfact.inventario.name != False :  
             worksheet.write(tf + ini, 11, todfact.inventario.name)
...
---------------------------------------- 4) EMPIEZA LA UNION
                            ...
                            (
                                ...
                                sl.usage,sl.complete_name,(sm.location_dest_id)as ubicacion,sm.write_uid,sm.inventory_id,sm.picking_id
                            ...
                            (
                                    ...
                                    ,(sm.product_qty *0)as v_entrada,(am.amount_total_signed )v_salida,(sm.product_qty  *0)v_saldo,
                                     sm.state,sm.origin,sm.reference ,sl.usage,sl.complete_name,(sm.location_id)as ubicacion,sm.write_uid,sm.inventory_id,sm.picking_id 
                            ...
                            (
                               select * from (
                                    ...
                                    ,(sm.location_id)as ubicacion,sm.write_uid,sm.inventory_id,sm.picking_id 
